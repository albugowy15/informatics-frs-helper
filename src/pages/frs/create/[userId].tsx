...
export default function CreateFRSPage() {
  ...
  const onSubmit: SubmitHandler<CreateFRSForm> = (data) => {
    if (classTaken.length > 0) {
      ...
      if (mutatePlan.isLoading) {
        toast.loading('Membuat rencana FRS...');
        return;
      }
      if (mutatePlan.isSuccess) {
        toast.success('Rencana FRS berhasil dibuat');
        return;
      }
      if (mutatePlan.isError) {
        toast.error('Gagal membuat rencana FRS: ' + mutatePlan.error.message);
        return;
      }
    }
  };
